...
// 記憶體資料庫（無 Redis 時全程 100% 順暢運作）
let memoryConfig = {
  prizes: [
    { name: "頭獎：這餐香香雞請你吃", prob: 5 },
    { name: "二獎：現折20元", prob: 25 },
    { name: "三獎：下次消費贈紅茶", prob: 25 },
    { name: "四獎：下次消費9折券", prob: 45 }
  ]
};
const memoryStorage = new Map();

// 初始化 Redis (若無連線將自動停用，不卡死任何請求)
const redis = createClient({
  url: process.env.REDIS_URL || 'redis://127.0.0.1:6379',
  socket: { connectTimeout: 1000, reconnectStrategy: false }
});
redis.on('error', () => {}); // 忽略無連線錯誤
redis.connect().catch(() => {});
...
app.get('/', (req, res) => {
  res.send(`
    <div style="text-align:center; padding:50px; font-family:sans-serif;">
      ...
      <p><a href="/admin" style="font-size:18px; color:#27ae60; font-weight:bold; text-decoration:none;">⚙️ 點此進入【後台管理控制台】</a></p>
      <p><a href="/clerk" style="font-size:18px; color:#2980b9; font-weight:bold; text-decoration:none;">📱 點此開啟【店員滾動 QR Code 畫面】</a></p>
    </div>
  `);
});
...
// -----------------------------------------------------------------------------
// 3. 店員端 - 動態滾動 QR Code 頁面 (直接由 Render 提供)
// -----------------------------------------------------------------------------
